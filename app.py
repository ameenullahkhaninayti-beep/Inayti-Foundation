...
def home():
    # Detect if the user clicked an upper navigation tab (default to 'home')
    current_tab = request.args.get("tab", "home")
    
    # Variables to track form responses
    ...
    if request.method == "POST":
        ...
        # --- IF USER SUBMITTED THE COUNSELING ASSESSMENT ---
        if form_type == "assessment":
            assessment_submitted = True
            current_tab = "home" # Keep them on the assessment view to see results
        ...
        # --- IF USER SUBMITTED THE CONTACT FORM ---
        elif form_type == "contact_form":
            contact_submitted = True
            current_tab = "contact" # Keep them on the contact view to see confirmation
            contact_name = request.form.get("c_name")
            contact_email = request.form.get("c_email")
            contact_msg = request.form.get("c_message")

    # Clean rendering variables to manage active states safely outside f-string template
    ...
    # Main Interface HTML & Styled Framework
    html_content = f"""
    <!DOCTYPE html>
    <html>
    <head>
        ...
        <style>
            ...
            .form-group input[type="text"], .form-group input[type="email"], .form-group input[type="number"], .form-group textarea {{ 
                width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 6px; box-sizing: border-box; font-size: 14px; background-color: #fff; transition: border-color 0.15s; 
            }}
            ...
            .radio-row label {{ cursor: pointer; font-size: 14.5px; }}
            
            button {{ background-color: #2563eb; color: white; border: none; padding: 12px 20px; border-radius: 6px; font-size: 15px; font-weight: 700; width: 100%; cursor: pointer; transition: background-color 0.2s; }}
            ...
            .result-container h3 {{ margin-top: 0; color: #155724; }}
            ...
            .step-card li {{ margin-bottom: 8px; font-size: 14px; }}
    ...
    # ================= TAB CONTENT #1: OVERVIEW & DIAGNOSTIC ASSESSMENT =================
    if current_tab == "home":
        html_content += """
                ...
                <p>Please input your structural details and objective path below to instantly calculate resource eligibility and review your next execution checkpoints.</p>
    ...
    # ================= TAB CONTENT #2: PRACTICE AREAS =================
    elif current_tab == "services":
        html_content += """
                <h2>Strategic Advisory Practice Areas</h2>
                <p>Our foundation delivers top-tier structural assistance across multi-disciplinary sectors, blending rigorous technical competence with personal consultation blueprints.</p>
                
                <div class="pillar-grid">
                    <div class="pillar-item">
                        <strong>Aerospace & Technological Advisory</strong>
                        High-level mapping for students specializing in aviation architecture, engineering fields, and advanced structural maintenance ecosystems.
                    </div>
                    <div class="pillar-item">
                    ...
                    <div class="pillar-item">
                        ...
                        Safe, highly confidential, non-judgmental digital architecture to navigate academic burnout, workspace anxiety, and performance fatigue.
                    </div>
                    <div class="pillar-item">
    ...
    # ================= TAB CONTENT #3: EXECUTIVE PROFILE (FOUNDER) =================
    elif current_tab == "founder":
        html_content += """
                <h2>Executive Profile: Our Founder</h2>
                <p>The core philosophy of the Inayti Youth Foundation is entirely built upon a simple, yet extraordinarily powerful driving conviction: <strong>"Start for helping others."</strong></p>
                
                <p>The comprehensive design of this digital portal is conceptualized and engineered under the exclusive vision of our founder, <strong>Ameen Ullah Khan Inayti</strong>.</p>
                
                <p>In a complex, highly fast-moving global architecture, young students, scaling developers, and modern working professionals frequently find themselves systemically isolated when attempting to resolve major vocational or structural mental health bottlenecks. Under his guidance, this portal serves as a radical bridge, turning chaotic hurdles into clear, executing milestones—guaranteeing that systemic boundaries never deter execution-driven minds.</p>
                
                <h3>Operational Core Pillars:</h3>
                <ul style="padding-left: 20px; line-height: 1.8; color: #475569; font-size:14px;">
                    ...
                    <li><strong>Immediate Action Items:</strong> Every diagnostic interaction produces instant execution steps, never jus
